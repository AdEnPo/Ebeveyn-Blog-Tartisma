      ...
      <ul class="navbar-nav mr-auto">
        ...
        <li class="nav-item dropdown">
          ...
          <div class="dropdown-menu" aria-labelledby="navbarDropdown">
            <a class="dropdown-item" href="#">Spamlı Postlar  </a>
            <a class="dropdown-item" href="#">Samlı Yorumlar</a>
            <a class="dropdown-item" href="#">Spamlı Yorum Cevapları</a>
...
<table class="table table-striped table-dark">
  <thead>
    <tr>
     ...
     <th scope="col">Yorum Sahibi</th>
      <th scope="col">Yorumun İçeriği</th>
      <th scope="col">Durum</th>
      <th scope="col">Tarih</th>
  ...
  <tbody>
  <?php
        while($sonuc = mysqli_fetch_assoc($spamCommentList))
        {  
            $userId = $sonuc["userId"];
            $userNameSorgu=mysqli_query($db,"select *from users where id='$userId'")->fetch_assoc();
            $userName=$userNameSorgu["userName"];
  ...
  <td> <a style="color:#27DE40" href="profileShow.php?userName=<?php echo  $userName ?>"><?php echo  $userName ?></td>
      ...
      <td>
         <a href="profileShow.php?userName=<?php /* Post Sahibi*/
              $commentId=$sonuc["commentId"];
              $yorumSahibi = mysqli_query($db,"select *from users where id=(Select userId from comments where id='$commentId')")->fetch_assoc();

              echo $yorumSahibi["userName"];?>"> <?php echo $yorumSahibi["userName"]; ?>   </a>
      </td>
      <td> <?php 
         $commentId=$sonuc["commentId"];
         $yorumText = mysqli_query($db,"select *from comments where id=$commentId")->fetch_assoc();
        echo $yorumText["text"];
      ...
      <td>
          <?php 
           $commentId=$sonuc["commentId"];
            if($sonuc["status"]=="0")
              echo "<a href='db/commentCheck.php?commentId=$commentId' class='btn btn-success btn-sm'>Tamamlandı</a>";
            else if($sonuc["status"]=="1")
              echo "Kontrol Edildi";
          ?>
      </td>
      ...
      <?php 
       $commentId=$sonuc["commentId"];
      $commentDurumSorgu = mysqli_query($db,"select *from comments where id='$commentId'")->fetch_assoc();
      $commentDurum = $commentDurumSorgu["status"];
      if($commentDurum==0)
      echo '<a href="db/commentBlock.php?commentId='.$commentId.'" class="btn btn-danger btn-sm">Yayından Kaldır</a>';
      else
      echo '<a href="db/commentUnblock.php?commentId='.$commentId.'" class="btn btn-success btn-sm"> Yayına Al</a>';
      ?>
...
<div class="row mt-4 mb-4">
              ...
              <div class="col-md-12 text-center">
            <h3> Spamlanan Yorum Cevapları </h3>
             </div>
...
<table class="table table-striped table-dark">
  ...
  <tbody>
  <?php
        while($sonuc = mysqli_fetch_assoc($spamCommentReplyList))
        {  
            $userId = $sonuc["userId"];
            $userNameSorgu=mysqli_query($db,"select *from users where id='$userId'")->fetch_assoc();
            $userName=$userNameSorgu["userName"];
  ...
  <td> <a style="color:#27DE40" href="profileShow.php?userName=<?php echo  $userName ?>"><?php echo  $userName ?></td>
      ...
      <td>
         <a href="profileShow.php?userName=<?php /* Post Sahibi*/
              $replyId=$sonuc["replyId"];
              $yorumSahibi = mysqli_query($db,"select *from users where id=(Select userId from commentsreply where id='$replyId')")->fetch_assoc();

              echo $yorumSahibi["userName"];?>"> <?php echo $yorumSahibi["userName"]; ?>   </a>
      </td>
      <td> <?php 
         $replyId=$sonuc["replyId"];
         $yorumText = mysqli_query($db,"select *from commentsreply where id=$replyId")->fetch_assoc();
        echo $yorumText["text"];
      ...
      <td>
          <?php 
            $replyId=$sonuc["replyId"];
            if($sonuc["status"]=="0")
              echo "<a href='db/commentReplyCheck.php?commentId=$replyId' class='btn btn-success btn-sm'>Tamamlandı</a>";
            else if($sonuc["status"]=="1")
              echo "Kontrol Edildi";
          ?>
      </td>
      ...
      <?php 
      $replyId=$sonuc["replyId"];
      $commentDurumSorgu = mysqli_query($db,"select *from commentsreply where id='$replyId'")->fetch_assoc();
      $commentDurum = $commentDurumSorgu["status"];
      if($commentDurum==0)
      echo '<a href="db/commentReplyBlock.php?replyId='.$replyId.'" class="btn btn-danger btn-sm">Yayından Kaldır</a>';
      else
      echo '<a href="db/commentReplyUnblock.php?replyId='.$replyId.'" class="btn btn-success btn-sm"> Yayına Al</a>';
      ?>
