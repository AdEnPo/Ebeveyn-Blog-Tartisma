...
$userList = mysqli_query($db,"select * from users");
 session_start();
...
<table class="table table-striped table-dark">
 ...
 <tbody>
 <?php
       while($sonuc = mysqli_fetch_assoc($userList))
       {  
           $userId = $sonuc["id"];  
 ...
 <td> <a style="color:#27DE40" href="profileShow.php?userName=<?php echo  $userName ?>"><?php echo  $userName ?></td>
     ...
     <?php 
     $userId=$sonuc["id"];
	 if($sonuc["status"] !=-2 && $sonuc["admin"]!=1)
     echo '<a href="db/blockUser.php?userId='.$userId.'" class="btn btn-danger btn-sm">Kullanıcıyı Engelle</a>';
     else
		if($sonuc["status"]==-2)
			 echo '<a href="db/unblockUser.php?userId='.$userId.'" class="btn btn-success btn-sm">Engeli Kaldır</a>';
		else
			echo '<span style="color:yellow">Admin<span>'
	 ?>
